...
export default function StatisticsPage({ accessToken }: { accessToken?: string }) {
            ...
            <Card className="mt-8 p-6 rounded-2xl shadow-md border-t-4 border-blue-600 bg-blue-50">
              <CardContent className="space-y-2 text-blue-900">
                ...
                <div className="grid grid-cols-2 md:grid-cols-4 gap-4 text-sm">
                  <div>Total Deposit Amount: <span className="font-semibold">{grandTotal.total_deposit_amount}</span></div>
                  <div>Total Withdraw Amount: <span className="font-semibold">{grandTotal.total_withdraw_amount}</span></div>
                  <div>Total Expense Amount: <span className="font-semibold">{grandTotal.total_expense_amount}</span></div>
                  <div>Total Balance: <span className={`font-semibold ${grandTotal.total_balance >= 0 ? "text-green-600" : "text-red-600"}`}>{grandTotal.total_balance}</span></div>
                </div>
              </CardContent>
            </Card>
          )}
